...
def delete_plan(pid: int, admin=Depends(require_admin)):   # sirf admin delete kar sakta hai
    ...
    with get_conn() as conn:
        cur = dict_cursor(conn)
        cur.execute("SELECT * FROM maintenance_daily_plan_work WHERE id=%s", (pid,))
        row = cur.fetchone()
        if not row:
            raise HTTPException(404, "Plan not found")

        from routers.maintenance_spare import clear_usage
        n_spare = clear_usage(conn, pid, "Daily Plan")

        cur.execute("DELETE FROM maintenance_daily_plan_work WHERE id=%s", (pid,))

        try:
            from main import write_audit
            write_audit(conn, action="DAILY_PLAN_DELETE", entity_type="maintenance_daily_plan_work", entity_id=pid,
            ...
            print(f"[DAILY] plan-delete ka audit nahi likha: {e}")
